...
**知识库的最小单元是卡片**。note 与 card 分属两个库，归属由来源决定：
...
**无形态演进（2026-08-04 定稿）**：note 与 card 是两个库的（personal/ 随笔 vs projects/ 模块），归属由来源决定——不存在"随笔成熟为模块卡"的演进。跨库移动只发生在归属错误时（如个人笔记实为项目知识，或反之）——那是归位（sync 结构审视处理），不是演进。
