...
九操作：init（全局初始化）、deinit（全局拆除）、import（项目入驻·新增）、migrate（主动迁移·老知识升级）、delete（项目退出）、sync（日常同步）、explore（知识检索）、note（随手笔记）、status（查看状态）。

触发方式：用户说 "qwiki <操作>"，加载本 skill 后识别操作名并执行。
