...
> 检索链的完整说明。概要由 hook 注入（inject.sh 每轮检索引导，方案 A 定稿）。
...
4. **模型自身知识** — 内核/协议/语言内置知识。
5. **web_search** — 外部网络搜索。

> 代码细节检索由各工具自身能力承担（search_files/read_file 或工具内置代码索引）——qwiki 只管理知识库本体，与 AGENTS.md / codegraph 无关（v1.9.0 解耦声明）。
...
sync 时自动执行：
- 遍历卡片「模块边界」中列出的文件路径
- search_files 验证文件是否仍存在
- 缺失 → INDEX 对应条目标记 ⚠️ 待验证，通知用户

> ⚠️ 腐化检测依赖卡片「模块边界」中的文件路径。已卡片化的项目参与检测；references/design 文档（无模块边界路径）待 migrate 卡片化后纳入。
